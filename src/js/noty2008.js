...
var xml2json = function(xml) {
  var f = function(xml) {
    ...
    for (var i = 0; i < xml.childNodes.length; i++) {
      ...
      if (name == "#text") text.push(node.textContent);
      else {
        hasxml = !0;
        if (json[name] == null) {
          json[name] = f(node)
...
var dump = function(o, name, target) {
  if (target == null) {
    debug("");
    target = get('debug')
  }
...
var setUI = function(comp) {
  var istouch = 'ontouchstart' in window;
  var usecapture = !1;
  if (istouch) {
    comp.addEventListener("touchstart", function(e) {
      if (this.onuidown != null)
        if (!this.onuidown((e.changedTouches[0].pageX - this.offsetLeft) * this.ratio, (e.changedTouches[0].pageY - this.offsetTop) * this.ratio)) e.preventDefault()
    }, usecapture);
    comp.addEventListener("touchmove", function(e) {
      if (this.onuimove != null)
        if (!this.onuimove((e.changedTouches[0].pageX - this.offsetLeft) * this.ratio, (e.changedTouches[0].pageY - this.offsetTop) * this.ratio)) e.preventDefault()
    }, usecapture);
    comp.addEventListener("touchend", function(e) {
      if (this.onuiup != null)
        if (!this.onuiup((e.changedTouches[0].pageX - this.offsetLeft) * this.ratio, (e.changedTouches[0].pageY - this.offsetTop) * this.ratio)) e.preventDefault()
    }, usecapture)
  }
...
var getContext = function(canvas) {
  ...
  g.init = function() {
    ...
    if (ua.indexOf("iPhone") >= 0 || ua.indexOf("iPad") >= 0 || ua.indexOf("iPod") >= 0) this.ratio = window.devicePixelRatio;
  ...
  g.fillArrow = function(x1, y1, x2, y2, arw, arh) {
    this.drawArrow(x1, y1, x2, y2, arw, arh, !0)
  };
  return g
}
